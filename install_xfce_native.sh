...
main() {
    ...
    echo -n "Please enter username for proot installation (lowercase letters, numbers, underscores, hyphens only): " > /dev/tty
    read username < /dev/tty
    
    # Validate username
    if [[ ! "$username" =~ ^[a-z_][a-z0-9_-]*$ ]]; then
        echo -e "${RED}Invalid username. Use only lowercase letters, numbers, underscores, and hyphens.${NC}"
        exit 1
    fi
    ...
    # Install core dependencies (added unzip)
    dependencies=('wget' 'proot-distro' 'x11-repo' 'tur-repo' 'pulseaudio' 'git' 'unzip')
    missing_deps=()
...
# The user can manually run these if needed:
echo -e "${YELLOW}Note: You may want to run 'termux-reload-settings' manually to apply all changes.${NC}"
